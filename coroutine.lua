...
-- @async_operation：异步句柄---或者任何带有isDone、progress成员属性的异步对象
-- @callback：每帧回调，传入参数为异步操作进度progress
local function waitforasync(async_operation, callback)
    waitforasyncop(async_operation, callback)
...
coroutine.waitforasync = waitforasync
...
coroutine.stopAction = stopAction
---- 调试用：查看内部状态
